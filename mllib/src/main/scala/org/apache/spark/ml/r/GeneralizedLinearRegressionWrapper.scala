...
private[r] object GeneralizedLinearRegressionWrapper
  ...
  def fit(
    ...
    val formula_ = new RFormula().setFormula(formula)

    log.warn("--- data ----")
    log.warn(data.columns.mkString(", "))

    val (newData, expandedFormulaString) = formula_.smoothTransformer(data)

    log.warn("--- newData ----")
    log.warn(newData.columns.mkString(", "))

    log.warn("changed formula string:")
    log.warn(expandedFormulaString)

    log.warn(newData.printSchema())
    val rFormula = new RFormula()
      .setFormula(expandedFormulaString)

    val rFormulaModel = rFormula.fit(newData)
    // get labels and feature names from output schema
    val schema = rFormulaModel.transform(newData).schema

    log.warn("---- schema ----")
    log.warn(schema)
    log.warn(schema.mkString(", "))

    val featureAttrs = AttributeGroup.fromStructField(schema(rFormula.getFeaturesCol))
      .attributes.get
    log.warn("---- featureAttrs ----")
    log.warn(featureAttrs)
    log.warn(featureAttrs.mkString(", "))
    val features = featureAttrs.map(_.name.get)
    // assemble and fit the pipeline
    log.warn("---- features ----")
    log.warn(features)
    log.warn(features.mkString(", "))
    ...
    log.warn("---- rFeaturesAttrs ----")
    log.warn(rFeatures.mkString(", "))

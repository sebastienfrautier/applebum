...
class RFormula @Since("1.5.0") (@Since("1.5.0") override val uid: String)
  ...
  @Since("2.0.0")
  def smoothTransformer(dataset: Dataset[_]) : (Dataset[_], String) = {
    val log = LogManager.getRootLogger
    require(isDefined(formula), "Formula must be defined first.")
    log.warn("INSIDE SMOOTH TRANSFORM")
    log.warn(($(formula)))
    log.warn(formula.toString)
    val parsedFormula = RFormulaParser.parse($(formula))
    val newData = parsedFormula.transform_spline(dataset).toDF()
    (newData , parsedFormula.explode_spline_terms)
  }
  @Since("2.0.0")

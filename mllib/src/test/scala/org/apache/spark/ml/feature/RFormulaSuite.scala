...
class RFormulaSuite extends SparkFunSuite with MLlibTestSparkContext with DefaultReadWriteTest {
  ...
  test("playground"){
    ...
    val original = spark.createDataFrame(
      Seq((1, 0.1, 0.3), (1, 0.2, 0.5),(1, 0.2, 0.50),(1, 0.2, 0.5))).toDF("gear", "disp", "wt")

      original.withColumn("ListNew", lit("0.2,0.3,0.4")).show()

      assert(true == true)
  }

  test("transform spline"){
    val original = spark.createDataFrame(
      Seq((1, 0.1, 0.3), (1, 0.2, 0.5),(1, 0.2, 0.50),(1, 0.2, 0.5))).toDF("gear", "disp", "wt")

    val formula_ = new RFormula().setFormula("gear ~ disp + cyl + s(wt, bs = cc, knots = c(2.01, 2.3, 2.6,     2.9, 3.1, 3.4, 3.7, 4.2, 4.2, 4.3, 4.7, 5.1))")
    println("------------ OLD FEATURE COL ---------")

    println(formula_.getFeaturesCol)

    val (newData, nFormula) = formula_.smoothTransformer(original)
    ...
    val formula = new RFormula().setFormula(nFormula)
    println("------------ OLD FEATURE COL ---------")
    println(formula.getFeaturesCol.toString)
    val model = formula.fit(newData)
    println(model.toString)
    val result = model.transform(newData)
    println(result.show())
    val resultSchema = model.transformSchema(newData.schema)

    val expected = spark.createDataFrame(
      Seq(
        (1, 0.1, 0.3, 0.03600000000000001, 0.0, 0.6306666666666666,0.08533333333333333),
        (1, 0.2, 0.5, 0.28266666666666673, 0.1666666666666666, 0.6666666666666667),
        (1, 0.2, 0.5, 0.28266666666666673, 0.1666666666666666, 0.6666666666666667),
        (1, 0.2, 0.5, 0.28266666666666673, 0.1666666666666666, 0.6666666666666667)
      )
    ).toDF("id", "v1", "v2", "v1_smooth_0", "v1_smooth_1", "v1_smooth_2","features","label")
